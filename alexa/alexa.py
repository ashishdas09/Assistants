    ...
    while True:
        ...
        if output:
            ...
            if "authorized" in str(output.strip()).lower():
                ...
                subprocess.Popen(["aplay", "{}/Assistants/sample-audio-files/AlexaStartup.wav".format(USER_PATH)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                #Comment the line below if you want to create your own indicator pattern
                assistantindicator('off')
            if "listening..." in str(output.strip().lower()):
                #Change the path to your desired audio file for the trigger alert tone
                subprocess.Popen(["aplay", "{}/Assistants/sample-audio-files/AlexaTriggered.wav".format(USER_PATH)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                #Comment the line below if you want to create your own indicator pattern
...
run_command("sudo {}/Assistants/alexa/startsample.sh".format(USER_PATH))
